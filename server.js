...
app.post('/chat', async (req, res) => {
    ...
    try {
      ...
      if (!stream && response && response.outputs) {
        ...
        // Format points (new line for bullets or numbered lists)
        let formattedOutput = output.replace(/(\d+\.\s|- )/g, "\n$1");
  
        // Highlight calculations
        formattedOutput = formattedOutput.replace(/\((\d+(\.\d+)?)\)/g, "**(Calculation: $1)**");
